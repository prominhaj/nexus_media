...
const StoriesModalCard = ({ story }) => {
    const cardContent = <>
        <div className='relative w-full h-full before:absolute before:w-full before:h-full before:bg-black/30 before:rounded-md'>
            <Image src={story?.storyPhoto} priority width={200} height={200} className='object-cover w-full h-full rounded-md' alt='Stories' />
            <div className='absolute z-10 w-full bottom-[8%]'>
                <div className='flex flex-col items-center gap-3'>
                    <Link href={`/profile/1`} className='block'>
                        <Avatar color="success" isBordered src={story?.profilePhoto} />
                    </Link>
    ...
    return (
        <>
            <StoryModal item={story} card={cardContent} />
        </>
    );
};
